...
export default function ScheduleFormBackup({
  visible,
  hideModal,
  instanceId,
}: props) {
  ...
  const backupSchedule = useSelector(selectBackupSchedule);
  const backupScheduleList = useSelector(selectBackupSchedules);
  const data = useSelector(selectData);

  useEffect(() => {
    dispatch(actions.setData({ instanceId }));
    dispatch(actions.getBackupSchedules());
  }, [dispatch, instanceId]);
  ...
  }, [dispatch, form, instanceId, backupSchedule]);
  ...
  const onFinish = values => {
    ...
    dispatch(
      actions.setData({ instanceId, data: { ...values, type: 'BACKUP' } }),
    );
    dispatch(actions.createBackupSchedule());
  };
  ...
  const handleClickDelete = (record, index) => {
    ...
    dispatch(actions.deleteScheduleBackup());
  };
  ...
  return (
    <>
      <Modal
        width={600}
        visible={visible}
        title={t('constant:Button.SCHEDULE_FORM_BACKUP')}
        onCancel={hideModal}
        footer={[
